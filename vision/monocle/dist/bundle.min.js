(function () {
	...
	var wsDriver = function () {
	    return xs.create({
	        start: function (listener) {
	            var ws = new WebSocket("ws://localhost:12131");
	...
	var StreamPrinter = {
	    ...
	    complete: function () { return console.log("StreamPrinter complete"); }
	};

	var status = ws_.filter(function (x) { return x.req === "status"; }).addListener(StreamPrinter);
	var devices = ws_.filter(function (x) { return x.req === "queryDevices"; }).map(function (x) { return x.res; }).addListener(StreamPrinter);

	devices.foreach(function (x) { return console.log(x); });

	var localStoreLookup = function (id) { return localStorage.getItem(id) === null ? false : true; };

	var localStoreTransact = function (obj) {
	    var key = "src" + (obj.id);
	    localStorage.getItem(key) === null ? localStorage.setItem(key, obj.id) : localStorage.removeItem(key);
	};

	var List = function (sources) {
	    // const props = devices;
	    ...
	    var isolateList = function (props) {
	    ...
	    var vdom_ = xs.combine.apply(null, isolateList(props))
	        .map(function (x) { return div(".list", x); });
	...
	var ListItem = function (sources) {
	    var domSource = sources.DOM;
	    var props_ = sources.Props;

	    var srcIds_ = domSource
	        .select(".src")
	        .events("click")
	        .map(function (ev) { return ({id: ev.target.id}); });
	    ...
	    var state_ = props_
	        .map(function (props) { return srcIds_
	            .map(function () { return ({id: props.id, label: props.label}); })
	            .startWith(props); }
	        )
	        .flatten();

	    var vdom_ = state_
	        .map(function (state) { return div(".list-item", [
	            span(".src", {
	                attrs: {id: state.id}
