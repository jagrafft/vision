(function () {
	...
	var main$1 = function (sources) {
	    ...
	    var status_ = dom
	        .select(".status")
	        .events("click")
	        .map(function (x) { return x.target.textContent; });
	    ...
	    var devices = ws_
	        .filter(function (x) { return x.req === "queryDevices"; })
	        .map(function (x) { return div(".device-list", [
	                h2("devices"),
	    ...
	    var status = ws_
	        .filter(function (x) { return x.req === "status"; })
	        .map(function (x) { return div(".status-list", [
	                h2("process status"),
	                span(".status", x.res)
	            ]); }
	        );

	    var vdom_ = xs.merge(devices, status);
	    return {
	        DOM: vdom_
	    }
	};
